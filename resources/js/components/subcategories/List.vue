                    ...
                    <template v-slot:cell(action)="row">
                        <b-button-group size="sm">
                            <b-button variant="primary"
                                      :to="{name:'SubcategoriesView',params:{id:row.item.id}}"
                                      @click="currentItem=JSON.parse(JSON.stringify(row.item))">
                                <i class="fa fa-eye"></i>
                            </b-button>
                            <b-button variant="warning"
                                      :to="{name:'SubcategoriesEdit',params:{id:row.item.id}}"
                                      @click="currentItem=JSON.parse(JSON.stringify(row.item))">
...
<script>
    ...
    export default {
        ...
        props: {
            ...
            api_url: {
                type: String,
                default: () => route('Backend.Subcategories.List').url()
            },
            trash_url: {
                type: String,
                default: () => route('Backend.Subcategories.Delete').url()
            },
            submit_url: {
                type: String,
                default: () => route('Backend.Subcategories.Store').url()
            },
            custom_buttons: {
                type: Array,
                default: () => [
