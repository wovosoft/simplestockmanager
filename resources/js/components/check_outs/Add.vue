<template>
    <b-modal @hidden="$router.go(-1),$emit('reset',true)"
             ...
             no-body
             :title="(form.id?'Edit ':'Add ')+'Check Out'"
             lazy>
        <form @submit.prevent="hitSubmit" ref="theForm">
            ...
            <b-form-group label="Note">
                <b-form-textarea placeholder="check-out Note" v-model="form.note"/>
            </b-form-group>
            <vue-select
            ...
            <b-table
                ...
                <template v-slot:cell(quantity)="row">
                    <b-form-input type="number" size="sm" v-model="row.item.quantity" :max="row.item.available"/>
                </template>
